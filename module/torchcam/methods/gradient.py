...
class _GradCAM(_CAM):
    ...
    def _backprop(self, scores: Tensor, class_idx: Union[int, List[int]], retain_graph: bool = False) -> None:
        ...
        # Handle tuple outputs from model (regression models may return (out, features))
        if isinstance(scores, tuple):
            scores = scores[0]

        # Backpropagate to get the gradients on the hooked layer
        if isinstance(class_idx, int):
            # For regression, we use class_idx=0 since there's only one output
            if scores.size(1) == 1:
                loss = scores.sum()
            else:
...
class SmoothGradCAMpp(_GradCAM):
    ...
    def _get_weights(
        ...
        if any(act is None for act in self.hook_a):
            raise RuntimeError("Some activation hooks are None. Make sure the model forward pass has been executed.")
        ...
        for _idx in range(self.num_samples):
            ...
            # Forward & Backward
            if text_included:
                out = self.model(noisy_input, text=text_tensor, text_included=True)
            else:
                out = self.model(noisy_input)
            self.model.zero_grad()
            self._backprop(out, class_idx, **kwargs)

            # Sum partial derivatives (only if all gradients are available)
            if all(grad is not None for grad in self.hook_g):
                ...
                print(f"Warning: Some gradients are None at iteration {_idx}: {[grad is None for grad in self.hook_g]}")
        ...
        # Check final gradients and apply pixel coefficient in each weight
        if all(grad is not None for grad in self.hook_g):
            return [a.mul_(torch.relu(grad)).flatten(2).sum(-1) for a, grad in zip(alpha, self.hook_g)]
        else:
            print(f"Error: Final gradients contain None values: {[grad is None for grad in self.hook_g]}")
            # Return zero tensors instead of empty list
            return [torch.zeros_like(act).flatten(2).sum(-1) for act in init_fmap]
